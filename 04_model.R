...
DBSAL_cor <- cor(DBSAL[,c(7:22, 24:30)], use = "complete.obs")
...
# Subset to 2014 and make predictions for 2015
DBSAL_one_year <- DBSAL_v2 %>%
  dplyr::filter(formatted_date >= as.Date("2014-01-01") & formatted_date < as.Date("2015-01-01"))
...
DBSAL_2015 <- DBSAL_v2 %>%
  dplyr::filter(formatted_date >= as.Date("2015-01-01") & formatted_date < as.Date("2016-01-01"))

my_pred5 <- predict(rf_model4, newdata = DBSAL_2015)
mse5 <- mean((my_pred5 - DBSAL_2015$salinity)^2)
r_squared5 <- 1 - sum((DBSAL_2015$salinity - my_pred5)^2) / sum((DBSAL_2015$salinity - mean(DBSAL_2015$salinity))^2)
...
my_pred7 <- predict(rf_model5, newdata = DBSAL_2015)
...
DBSAL_other_subset <- DBSAL_v2 %>%
  dplyr::filter(formatted_date >= as.Date("2020-01-01") & formatted_date < as.Date("2022-01-01"))
...
rf_index.sdesign.vsurf <- VSURF(train_all_years[, c(7:22, 24:30)], 
...
# VSURF chose distCoast, slope, B03, SI, B04, NDSI, B02, srad, SRSI, tavg
train_all_years[, c(7:22, 24:30)][rf_index.sdesign.vsurf$varselect.pred]
